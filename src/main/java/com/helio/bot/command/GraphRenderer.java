...
/** Простой рендер линейного графика онлайна во времени (PNG). */
public final class GraphRenderer {
    ...
    public static byte[] render(String title, Map<String, List<StatsStore.Sample>> series,
        ...
        int maxY = 1;
        for (List<StatsStore.Sample> s : series.values()) {
            for (StatsStore.Sample p : s) {
        ...
        for (Map.Entry<String, List<StatsStore.Sample>> e : series.entrySet()) {
            Color col = COLORS[ci % COLORS.length];
            ci++;
            g.setColor(col);
            int prevX = -1, prevY = -1;
            for (StatsStore.Sample p : e.getValue()) {
